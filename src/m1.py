...
class CircleChanger(object):
    ...
    def __init__(self, x, y, radius, fill_color, colors):
        ...
        """
        self.animation_factor = 5  # Smaller => faster animations
    ...
    def swallow(self, other_circle_changer):
        ...
        # new center
        this_center = self.circle.center
        other_center = other_circle_changer.circle.center
        new_center = this_center.halfway_to(other_center)

        # new radius
        new_radius = other_center.get_distance_from(self.circle.center)/2

        # new 'colors' tuple
        new_colors = self.colors + other_circle_changer.colors

        # construct and return
        new_cc = CircleChanger(new_center.x, new_center.y, new_radius, 'red', new_colors)
        return new_cc
    ...
    def change_to_next_color_in_tuple(self):
        ...
        self.fill_color_index += 1
        if self.fill_color_index >= len(self.colors):
            self.fill_color_index = 0

        self.change_color(self.fill_color_index)
